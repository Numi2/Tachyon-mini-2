...
impl Batch {
    ...
    pub fn derive_metadata_bytes(&self) -> [u8; 32] {
        ...
        let mut buf = Vec::with_capacity(16 + 32 + 32 + 16);
        buf.extend_from_slice(b"ragu-wallet:meta");
...
impl Circuit<FrVesta> for WalletCircuit {
    ...
    fn synthesize<D: Driver<FrVesta, Var = crate::cs::Var>>(&self, d: &mut D, input: Self::Input) -> Self::Output {
        let lsb_even = (PrimeField::to_repr(&input.metadata).as_ref()[0] & 1) == 0;
        if lsb_even {
            ...
            // Alternate path: enforce_zero on (accumulator - accumulator) = 0
            let acc = d.input_public(input.accumulator);
