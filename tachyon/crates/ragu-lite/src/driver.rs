...
impl<F: PrimeField> CpuDriver<F> {
    pub fn new() -> Self {
        Self { cs: ConstraintSystem::default(), values: Vec::new() }
    }

    #[inline]
    fn push_value(&mut self, v: F, public: bool) -> Var {
        let var = if public { self.cs.alloc_public() } else { self.cs.alloc() };
        if var.0 as usize == self.values.len() {
            self.values.push(v);
        } else {
            self.values[var.0 as usize] = v;
        }
        var
    }
...
impl<F: PrimeField> Driver<F> for CpuDriver<F> {
    ...
    fn add(&mut self, a: Self::Var, b: Self::Var) -> Self::Var {
        ...
        // enforce: out - a - b = 0
        self.enforce_zero(
            LinComb { terms: vec![(out, F::ONE), (a, -F::ONE), (b, -F::ONE)], constant: F::ZERO },
        );
        out
    }
    ...
    fn scale(&mut self, a: Self::Var, s: F) -> Self::Var {
        let va = self.value(a);
        let out = self.witness(va * s);
        // enforce: out - s*a = 0
        self.enforce_zero(
            LinComb { terms: vec![(out, F::ONE), (a, -s)], constant: F::ZERO },
        );
        out
    }
    ...
    fn instance(&self) -> Instance<F> {
        ...
        for &v in &self.cs.public_inputs {
            inputs.push(self.values[v.0 as usize]);
        }
        Instance { inputs }
    }
}
...
pub trait Circuit<F: PrimeField> {
    ...
    fn synthesize<D: Driver<F, Var = crate::cs::Var>>(&self, d: &mut D, input: Self::Input) -> Self::Output;
}
